    ...
    <main className="bg-[var(--page-bg)] text-white">
      <section className="relative overflow-hidden border-b border-white/10">
        ...
        <motion.header
          ...
          className="relative z-20 border-b border-white/10 bg-[#0a0a0c]/95 backdrop-blur-md"
        >
          <div className="relative flex h-20 w-full items-center justify-between px-4 sm:px-6">
            ...
            <nav className="absolute left-1/2 top-1/2 z-10 hidden -translate-x-1/2 -translate-y-1/2 items-center gap-8 text-sm font-medium md:flex">
              {[
                ...
                <motion.a
                  ...
                  className={
                    item.active
                      ? 'border-b-2 border-[var(--brand-lavender)] pb-1 text-[var(--brand-lavender)]'
            ...
            <motion.div
              ...
              animate={{ opacity: 1, x: 0 }}
              transition={{ duration: 0.6, delay: 0.3 }}
              className="relative z-10 flex shrink-0 items-center gap-5"
            >
              <button
                type="button"
              ...
              <motion.button
                type="button"
                whileHover={{ scale: 1.05, boxShadow: '0 0 25px rgba(233,221,255,0.4)' }}
                whileTap={{ scale: 0.97 }}
                className="rounded-full bg-[#E9DDFF] px-6 py-2.5 text-sm font-semibold text-[#1a1523] transition-shadow"
              >
                Get Started
              </motion.button>
            </motion.div>
        ...
        <div className="relative z-10 w-full px-4 pt-12 pb-16 text-left sm:px-6 sm:pb-20 lg:px-6 lg:pb-48 lg:pr-[22vw] lg:pt-48 xl:pr-[344px]">
          ...
          >
            <motion.div
              ...
              className="mb-8 inline-flex rounded-full border border-white/15 bg-[#363437] px-4 py-1.5 text-[11px] font-semibold uppercase tracking-[0.18em] text-[#82E9FF] animate-shimmer"
              style={{ backgroundImage: 'linear-gradient(110deg, transparent 25%, rgba(130,233,255,0.12) 50%, transparent 75%)' }}
            >
            ...
            <h1 className="font-black tracking-tight text-white">
              {[
                { text: 'Fill Your Venue', gradient: false, size: 'large' },
                { text: 'Every Night', gradient: false, size: 'large' },
                { text: 'Predict, Target,', gradient: true, size: 'medium' },
                { text: 'and Convert High-', gradient: true, size: 'medium' },
                { text: 'Value Guests', gradient: true, size: 'medium' },
              ].map((line, i) => (
                <motion.span
                  key={i}
                  variants={{ hidden: { opacity: 0, y: 40, filter: 'blur(8px)' }, show: { opacity: 1, y: 0, filter: 'blur(0px)' } }}
                  transition={{ duration: 0.8, ease: [0.22, 1, 0.36, 1] }}
                  className={`${i > 0 && !line.gradient ? 'mt-1 lg:mt-2' : ''} ${i === 2 ? 'mt-2 sm:mt-3 lg:mt-4' : ''} block ${
                    line.gradient
                      ? 'bg-gradient-to-r from-[#E9DDFF] to-[#82E9FF] bg-clip-text text-[clamp(1.75rem,5.5vw,4.25rem)] leading-[1] text-transparent lg:text-[72px] lg:leading-[1.02]'
                      : 'text-[clamp(2.25rem,8vw,5.5rem)] leading-[0.98] sm:leading-[0.98] lg:text-[96px] lg:leading-[0.95]'
                  }`}
                >
                  {line.text}
                </motion.span>
            ...
            <motion.p
              ...
              className="mt-8 max-w-xl text-base leading-relaxed text-white sm:text-lg lg:mt-10 lg:text-xl"
            >
            ...
            >
              <motion.button
                ...
                whileTap={{ scale: 0.97 }}
                className="animate-cta-glow rounded-full bg-[#E9DDFF] px-8 py-3 text-sm font-bold text-[#1a1523] transition-transform sm:px-10 sm:py-5 sm:text-base"
              >
              ...
              <motion.button
                ...
                whileTap={{ scale: 0.97 }}
                className="rounded-full border border-white/25 bg-[rgba(43,41,44,0.4)] px-8 py-3 text-sm font-semibold text-white transition-colors sm:px-10 sm:py-5 sm:text-base"
              >
                See How It Works
              </motion.button>
            </motion.div>
      ...
      <section className="border-y border-white/[0.05] bg-[#0F0E10]">
        <div className={`${contentRail} py-16`}>
          <div className="grid grid-cols-2 gap-x-4 gap-y-10 md:grid-cols-4 md:gap-x-6">
            {(
              [
                { target: 94000, suffix: '+', label: 'BOOKINGS GENERATED', tone: 'white' as const, isText: false },
                { target: 0, suffix: '', label: 'USED BY TOP VENUES', tone: 'cyan' as const, isText: true, text: 'Dubai' },
                { target: 32, suffix: '%', prefix: '+', label: 'TABLE BOOKING INCREASE', tone: 'white' as const, isText: false },
                { target: 18, suffix: '%', prefix: '+', label: 'SPEND PER GUEST', tone: 'cyan' as const, isText: false },
              ]
            ).map((stat, i) => (
              <Reveal key={stat.label} delay={i * 0.12} className="text-center md:text-center">
                <p
                  className={`text-xl font-bold leading-none sm:text-2xl lg:text-[34px] ${stat.tone === 'cyan' ? 'text-[#82E9FF]' : 'text-white'}`}
                >
                  {stat.isText ? stat.text : <Counter target={stat.target} suffix={stat.suffix} prefix={stat.prefix ?? ''} />}
                </p>
          ...
          <div className="flex flex-col gap-6 lg:gap-8">
            <div className="grid grid-cols-1 gap-6 lg:grid-cols-5 lg:gap-8 [&>*]:min-w-0">
              <Reveal className="lg:col-span-3">
                <motion.article
                  whileHover={{ y: -6, borderColor: 'rgba(233,221,255,0.18)' }}
                  transition={{ duration: 0.35 }}
                  className="relative flex min-h-[320px] flex-col overflow-hidden rounded-[28px] border border-white/[0.08] bg-[#1c1c1c] p-8 sm:min-h-[360px] sm:p-10 lg:min-h-[380px]"
                >
                  <div
                    className="pointer-events-none absolute -bottom-24 -right-24 z-0 h-[320px] w-[320px] rounded-full bg-[#E9DDFF] animate-glow-pulse"
              ...
              <Reveal delay={0.15} className="lg:col-span-2">
                <motion.article
                  whileHover={{ y: -6, borderColor: 'rgba(130,233,255,0.2)' }}
                  transition={{ duration: 0.35 }}
                  className="relative flex min-h-[320px] flex-col overflow-hidden rounded-[28px] border border-white/[0.08] bg-[#1c1c1c] p-8 sm:min-h-[360px] sm:p-10 lg:min-h-[380px]"
                >
                  <div className="relative z-10 flex flex-col text-left">
            ...
            <Reveal delay={0.1}>
              <motion.article
                whileHover={{ y: -4, borderColor: 'rgba(233,221,255,0.15)' }}
                transition={{ duration: 0.35 }}
                className="grid gap-8 rounded-[28px] border border-white/[0.08] bg-[#1c1c1c] p-8 sm:gap-10 sm:p-10 lg:grid-cols-12"
              >
      ...
      <section className="w-full bg-[#0B0B0B]">
        ...
        <Reveal>
          <motion.div
            whileHover={{ borderColor: 'rgba(233,221,255,0.35)' }}
            transition={{ duration: 0.4 }}
            className="mx-auto w-full max-w-[1120px] rounded-[24px] border border-[rgba(233,221,255,0.2)] bg-[rgba(43,41,44,0.4)] p-8 sm:p-12"
          >
          <div className="flex flex-col gap-8 lg:grid lg:grid-cols-2 lg:items-start lg:gap-x-10 xl:gap-x-12">
            <div className="order-2 min-w-0 lg:order-1 lg:max-w-[28rem] lg:self-center">
              ...
              <motion.button
                type="button"
                whileHover={{ scale: 1.05, boxShadow: '0 0 30px rgba(233,221,255,0.3)' }}
                whileTap={{ scale: 0.97 }}
            ...
            <div className="order-1 flex min-w-0 flex-col lg:order-2">
              <div className="mb-4 flex justify-end lg:mb-3">
                <p className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.14em] text-[var(--live-red)] sm:text-sm">
                  <span className="inline-block h-2 w-2 shrink-0 rounded-full bg-[var(--live-red)] animate-live-pulse" aria-hidden />
              ...
              <div className="flex flex-col gap-4 rounded-2xl border border-white/[0.05] bg-black/40 p-6">
                ...
                <div>
                  ...
                  <div className="mt-2 h-2.5 rounded-full bg-white/10 sm:h-3">
                    <div
                      className="h-full rounded-full bg-[#E9DDFF] animate-bar-fill"
        ...
        <div className="mt-20 grid grid-cols-1 gap-12 md:grid-cols-3">
          {[
            ...
            <Reveal key={step.title} delay={i * 0.18}>
              <motion.article
                whileHover={{ y: -8 }}
              ...
              >
                <motion.div
                  whileHover={{ scale: 1.1, borderColor: 'rgba(233,221,255,0.3)' }}
                  className="mb-6 flex h-16 w-16 shrink-0 items-center justify-center rounded-full border border-white/10 bg-[#252525] transition-colors"
                >
                  <img
                    src={step.iconSrc}
                    alt={step.iconAlt}
          ...
          <div className="grid gap-5 md:grid-cols-3 md:gap-6 lg:items-stretch">
            ...
            <Reveal delay={0}>
              <motion.article
                ...
                transition={{ duration: 0.35 }}
                className="flex h-full flex-col rounded-[32px] border border-white/[0.05] bg-[#141316] p-8 sm:p-10"
              >
                ...
                <motion.button
                  ...
                  whileTap={{ scale: 0.97 }}
                  className="mt-8 w-full rounded-full border border-white/25 bg-transparent py-3.5 text-base font-semibold text-white transition sm:py-4"
                >
                  Select Plan
                </motion.button>
              </motion.article>
            ...
            <Reveal delay={0.15}>
              <motion.article
                whileHover={{ y: -8, boxShadow: '0 0 60px -8px rgba(233,221,255,0.45)' }}
                transition={{ duration: 0.35 }}
                className="relative flex h-full flex-col rounded-[32px] border-2 border-[#E9DDFF] bg-[#2B292C] p-8 shadow-[0_0_48px_-8px_rgba(233,221,255,0.35)] sm:p-10 animate-border-glow"
              ...
              >
                ...
                <motion.button
                  type="button"
                  whileHover={{ scale: 1.03, boxShadow: '0 0 25px rgba(233,221,255,0.35)' }}
                  whileTap={{ scale: 0.97 }}
